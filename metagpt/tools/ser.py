...
def serialize(o: object, depth: int = 0) -> U | None:
    ...
    if isinstance(o, tuple):
        return tuple(serialize(d, depth + 1) for d in o)
...
"""
o: {'__classname__': '__main__.U', '__version__': 1, '__data__': {'x': 1, 'w': {'x': 2}, 'u': (1, 2)}}
__main__.U 1 {'x': 1, 'w': {'x': 2}, 'u': (1, 2)}
import_string: <class '__main__.U'>
...
def deserialize(o: T | None, full=True, type_hint: Any = None) -> object:
    ...
    if CLASSNAME not in o and not type_hint or VERSION not in o:
        x = {str(k): deserialize(v, full) for k, v in o.items()}
        print('plain dict', x)
        return x
